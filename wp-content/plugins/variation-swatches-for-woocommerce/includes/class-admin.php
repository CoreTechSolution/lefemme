	...
	public function attribute_fields( $type, $value, $form ) {
		...
		switch ( $type ) {
			...
			case 'color':
                ...
                <?php
                    $another_color_check = get_term_meta($_GET['tag_ID'], 'another_color_check', true);
                    $color1 = get_term_meta($_GET['tag_ID'], 'color1', true);
                ?>
                <input<?php if($another_color_check == 1) { echo ' checked="checked"'; } ?> id="another_color_check" name="another_color_check" type="checkbox" value="1" />&nbsp;&nbsp;Use another
                color.
                <div style="margin-top: 8px;" id="another_color" data-html='<input type="text" id="term-color1" name="color1" value="<?php echo esc_attr( $color1 ) ?>" />'></div>
				<?php
				break;
	...
	public function save_term_meta( $term_id, $tt_id ) {
		foreach ( TA_WCVS()->types as $type => $label ) {
			if ( isset( $_POST[$type] ) ) {
				...
				if($type == 'color') {
				    if($_POST['another_color_check'] == 1) {
					    update_term_meta( $term_id, 'another_color_check', 1 );
					    update_term_meta( $term_id, 'color1', $_POST['color1'] );
                    } else {
					    update_term_meta( $term_id, 'another_color_check', 0 );
					    update_term_meta( $term_id, 'color1', '' );
                    }
                }
			}
		}
		...
		switch ( $attr->attribute_type ) {
			case 'color':
				...
				if($another_color_check == 1) {
					echo '<div class="swatch-preview swatch-color" style="background-color: '.$value.'; background-image: -webkit-linear-gradient(30deg, '.$value.' 50%, '.$color1.' 50%);"></div>';
				} else {
