	...
	public function swatch_html( $html, $term, $attr, $args ) {
		...
		switch ( $attr->attribute_type ) {
			case 'color':
				...
				$another_color_check = get_term_meta($term->term_id, 'another_color_check', true);
				$color1 = get_term_meta($term->term_id, 'color1', true);
				list( $r, $g, $b ) = sscanf( $color, "#%02x%02x%02x" );
				if($another_color_check == 1) {
					$html = '<span class="swatch swatch-color swatch-'.esc_attr( $term->slug ).' '.$selected.'" style="background-color:'.esc_attr( $color ).';background-image: -webkit-linear-gradient(30deg, '.esc_attr( $color ).' 50%, '.esc_attr( $color1 ).' 50%);color:rgba('.$r.','.$g.','.$b.',0.5);" title="'.esc_attr( $name ).'" data-value="'.esc_attr( $term->slug ).'">'.$name.'</span>';
				} else {
					$html = '<span class="swatch swatch-color swatch-'.esc_attr( $term->slug ).' '.$selected.'" style="background-color:'.esc_attr( $color ).';color:rgba('.$r.','.$g.','.$b.',0.5);" title="'.esc_attr( $name ).'" data-value="'.esc_attr( $term->slug ).'">'.$name.'</span>';
				}
				/*list( $r, $g, $b ) = sscanf( $color, "#%02x%02x%02x" );
				$html = sprintf(
